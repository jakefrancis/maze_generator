...
const genMaze = (rows, columns, board) => {
  const search = (r, c, direction, depth) => {
    if (c === board.length - 1 && r === board[0].length - 1) {
      ...
      //wall gone
      ctx.fillRect(
        r * blockHeight + halfBlock + subLength,
        c * blockHeight + halfBlock,
        2 * halfBlock,
        subLength
      );
    }
    ...
    if (board[c][r] === "0") {
      ...
      switch (direction) {
        ...
        case "up":
          ...
          ctx.fillRect(
            r * blockHeight + halfBlock,
            c * blockHeight + subLength + halfBlock,
            subLength,
            2 * halfBlock
          );
          //visited cell
...
let randomMaze = genMaze(
  0,
  0,
  board(width / blockHeight, height / blockHeight)
);
